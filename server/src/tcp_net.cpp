...
int TcpNet::InitNetWork()
{
    ...
    if (inet_pton(AF_INET, kServerIp, &serveraddr.sin_addr.s_addr) == -1)
    {
    ...
    //监听socket
    if (listen(sockfd, kListenCount) == -1)
    {
        perror("Listen Error!");
        return false;
    }
    epfd = epoll_create(kEpollSize); //创建epoll的句柄，可以监听的文件描述符为_DEF_EPOLLSIZE
...
void *TcpNet::EPOLL_Jobs(void *arg)
{
    ...
    while (1)
    {
        //阻塞-1监听socket
        //printf("%d\n", i++);
        if ((ready = epoll_wait(m_pThis->epfd, m_pThis->epollarr, kEpollSize, -1)) == -1)
...
void TcpNet::Epoll_Deal(int ready, pool_t *pool)
{
    ...
    for (i = 0; i < ready; i++)
    {
        ...
        {
            Deletefd(fd);
            m_pool->Producer_add(pool, Info_Recv, reinterpret_cast<void *>(fd));
        }
    }
}
...
void *TcpNet::Info_Recv(void *arg)
{
    ...
    //cout << "nRelReadNum=" << nRelReadNum << endl;
    if (nRelReadNum <= 0)
    {
        close(clientfd);
        return NULL;
