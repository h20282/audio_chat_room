/*
 * @Author: FengYanBin
 * @Date: 2021-08-17 10:02:39
 * @LastEditors: FengYanBin
 * @LastEditTime: 2021-08-18 17:16:57
 * @Description: file content
 * @FilePath: \sql\audio\audioplayer.cpp
 */
...
void AudioPlayer::run()
{
    ...
    while (!this->isInterruptionRequested() and m_playing)
    {
        ...
        if (m_output->bytesFree() >= sizeof(AudioFrame))
        {
            ...
            {
                //qWarning() << "a empty frame!(len == 0)";
                QThread::msleep(5000);
            }
            else
            {
                auto writeCnt = m_audioIo->write(frame.buff, frame.len);
                //qDebug() << writeCnt << "bytes audio writen";
            }
        }
    }
